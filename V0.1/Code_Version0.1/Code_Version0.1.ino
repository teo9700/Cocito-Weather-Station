...
float pres, temp, hum, temp2, winds, windd, solvolt, battvolt;
...
byte minutes;
volatile unsigned long raintime, rainlast, raininterval, rainBegin;
float rainTot = 0;
...
void loop() {
  ...
  if (temp == 0) {                                                   //sometimes the sensor gives 0 as an error, we check 3 times to mame sure the temperature is correct
    temp = am2315.readTemperature();
  }
  if (temp == 0) {
    temp = am2315.readTemperature();
  ...
  solvolt = averageAnalogRead(solPin) * 0.0051 * (102 / 20);            //you need to manually calibrate the adc
  battvolt = averageAnalogRead(battPin) * 0.0051 * (102 / 20);

  upload(apiKey1, pres, temp, hum, temp2, winds, windd, rainTot);        //(apikey,field1,field2,field3,field4,field5,field6,filed7)
...
void connectWiFi() {
  ...
  delay(2000);
  esp8266.println("AT+CWMODE=1");      //sta mode
  delay(2000);
...
// Activated by the magnet and reed switch in the rain gauge, attached to input D2
{
  ...
  {
    rainTot += 0.2794; //Each dump is 0.011" of water
